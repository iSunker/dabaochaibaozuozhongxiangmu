...
14. ⬜ **2026-09-12 晚：「漂移哨兵 + 推前扫描」这一轮的收尾与遗留**
   ...
   * ✅ **暂存区已真删（2026-09-13）** —— `_cleanup-20260912/` 与
     ...
     **6 个 `.env.bak.*` 是生产 `.env` 的明文副本（带全部凭据）**；后者 8 个文件。
     ...
     手动跑一次即可）。它默认只列清单、`--apply` 才删；**先删 `.env.bak.*` 再删目录**；
     ...
     ★ **已知窄口（2026-09-13 实测）**：闸 ⑤ 第 1 步只匹配 `.env.bak.*`，而
     `#recycle/env-bak-20260912/` 那 8 个里**只有 7 个**匹配（第 8 个是别的命名）。
     这次没出事——第 2 步 `rm -rf` 整目录把它一起带走了——**但脚本头部那句
     「哪怕中途断电，凭据也已经先走了」对这个命名变体不成立**。要补就是把
     计数与删除两处的 `.env.bak.*` 都放宽成 `.env.bak*`。
