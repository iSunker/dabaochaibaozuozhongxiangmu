...
>
> ★ **还有一条：任务脚本别把输出重定向到 `/volume1`。**
>   那个卷 **剩余 0 字节**（2026-09-13 实测，见 SUMMARY §21.7）。
>   重定向过去 = 每一行 `echo` 都失败 —— 而脚本是 `set -e` 的，
>   **打印一行进度失败会把整趟任务带走**。这正是邮件风暴的触发条件
>   （详见 SUMMARY §21.7）。就这么干：**让 stdout 交给 DSM 自己处理**，
>   别加 `> /volume1/xxx.out` 这类重定向；要看输出就看下面那些日志文件。
>
