...
> | 格 | 期望值 / 基线 | 它指回的真实记录 |
> |---|---|---|
> | `fa` / `fb` / `fd` | `a == b == 1011`，`fd = a − b = 0` | 当日 `info.current.log` 里的 `Found` 行 |
> | `fb_c_all` / `fb_c_farm` | `0` / `1`（全量口径 / 农场行计数） | `hlink/state.db` 的 `pack`+`movie` |
> | `unclaimed` | **基线 1**（`0观影清单chrlee整理`） | 农场里那个只有 `.xlsx` 的目录 |
> | `packs_undriven` / `packs_unreg` | **基线 1**（`mbf` 登记了没被驱动） | `--packs` 的实际取值 vs `pack` 表 |
>
...
> * **`unclaimed` 报 `0` ≠ "判据没跑过"** —— 那是**跑了、且和基线 1 矛盾** ⇒
>   去查那条真实记录还在不在（农场的 `.xlsx` 被删了？）。
> * **`packs_undriven` / `packs_unreg` 只报「变化」，不报「现状」**（2026-09-12 深夜改）：
>   现状（如 `mbf` 登记了却一直没被驱动）是**已接受**的，非空就每天喊一次只会把
>   告警喊成噪音 —— 而噪音的代价是真的出问题时没人看。规则：
>   **差集里出现基线里没有的 → 发 alert**；与基线一致、或缩回基线之内 → 不发。
>   基线存在 `.reconcile.state` 的 `_packs_baseline` 里（**不进代码** —— 写进代码就
>   分不清「回到基线」和「判据死了」），缩也采纳新基线，所以它**再长回来会被抓住**。
>   ★ **不告警 ≠ 看不见**：日报正文每天照旧打印完整差集。
>   ★★ **不告警也 ≠ 跑了** —— alert 沉默有**四个**来源：
>   ① 与基线一致 ② 缩回基线之内 ③ 状态库读不到（正文写「没跑成」）
>   ④ 调用方没给 `--packs`（正文写「跳过」）。**后两种同样不发 alert**。
>   所以「沉默 = 没事」**要多读一步**：看正文那一行**有没有值 / 是不是 `n/a`**。
>   反过来，**响铃不需要附加判断** —— 响 ⟺ 真的算了差集、且真有新增。
...
> `unclaimed-searchee`、`packs-mismatch`。
