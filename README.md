...
| **11** | **收拢 `:3060` 上 22 条指向旧根的种子** ✅ **已完成** —— 902 条里 880 在新根 / **22 在旧根**（南洋 14 + HDFans 8，**全是 IYUU 的** tag，我们 599 条一条不漏已在新根）。`--all-tags` 收拢：预检 → `--limit 1` 试跑 → 全量，**902/902 全在新根，0 条指旧根**。★ 那 19 条 `error` 是**搬之前就 error 的**（试跑只对 2 个 hash 下过 setLocation）；error 总数 23→24（+1，未归因）。⬜ **剩两件收尾**：① 旧根盘上还留 **52.42 GB / 10 条目**（8 条与新根同名=重复，1 条 `149.V字仇杀队…` 只此一份）—— 现在**无任何种子引用**，但**不能断言占 52 GB 实空间**（硬链接可能共享 inode），**必须在 NAS 上** `du -sh` / `find -samefile` 判，且**只许在 NAS 上删** ② `qbittorrent-reseed` 的 `/downloads` **没挂载**（只挂了 `/downloads/incomplete`）而上限默认保存路径是 `/downloads/` ⇒ 任何不带 savepath 的添加会落进**容器可写层**（宿主机看不见、重建即丢）—— 现状无害（cross-seed/IYUU 都显式传 savepath），是**潜伏雷** | §21.4 / §21.5 / §21.6 |
