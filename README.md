...
> ★ **更正（2026-09-20，README 拆分）**：**README 已瘦身成 GitHub 门面** ——
> 只留「**怎么用**」（免责声明 · 目录结构 · 部署步骤 · 验证清单 · 常见问题），
> 通篇是 `YOUR-NAS` / `NAS_IP` / `SiteA` 占位符，**可以安全公开**。
> ⇒ 原先塞在这里的**运维事实**（真实路径 · 调度 · 日报读数基线 · 状态机语义 ·
> 通知链路 · 农场 · 漂移哨兵 · 凭据扫描 · 当前状态）**已整段搬进 `ENVIRONMENT.md` 的
> `A.13`–`A.19`**（那里通篇是真实值，**别公开贴出**）。上面撤节处都留了指针。
...
> ★ **更正（2026-09-20）**：它**仍是检索层**（`ERR-*` 可 grep），但**运维事实的真源已经搬进它**
> （新的 `A.13`–`A.19`）—— 因为 README 不再承担事实源。
> ⇒ **现在的优先级**：运维事实**以 `ENVIRONMENT.md` 为准**；原理与过程记录仍以 `SUMMARY` 为准。
