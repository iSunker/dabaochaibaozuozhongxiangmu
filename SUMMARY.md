...
### 21.5 旧根残留：★ 已判明 —— **9/9 全是独立副本**，删了真释放 **48.20 GiB**（2026-09-13）
...
三者**表现完全一样、修复完全一样**，所以不做区分也能修。要分开它们需要看那趟
DSM 任务计划把 stdout 重定向到了哪 —— 见文末「遗留」。
...
#### ★ 遗留：触发条件仍未定死（需要 NAS 侧两个读数）

修复对三种成因都成立，所以**不影响已做的改动**。要把它们分开，需要：

1. **那趟排空任务计划里 stdout 被重定向到了哪**（或干脆没重定向）——
   判据：若重定向到 `/volume1` 下，则"`say` 撞满盘"成立。
2. **风暴邮件的主题行**：预期是 `[reseed] 告警：站点退避中：HDtime`。
   若**不是**这个形状，那风暴来自另一条通道（例如 DSM 的「发送运行详情」，
   那是**288 封/天**），与本文档讲的不是同一件事。

> ★ **另一个必须说明的读数**：`archive/` 里那对 12:40 文件**同时具备**
> 「已归档」与「没有日志行」两个特征 —— 而**代码里两条分支都是先 `log_event` 再 `mv`**
> （实验已证），所以**这条组合在本脚本的正常路径上不存在**。
> ⇒ 13:10:02 那次 spool→archive **不是 `do_drain` 干的**（`send_mail` 失败时是
> `continue`、不归档；`say` 失败时整趟退出、也不归档），**大概率是人手动挪的**。
> 这条要问用户确认 —— 它不影响根因（根因是那 30 分钟里被重发的 6 封），
> 但**不能把它当成"脚本自己归档的"写进结论**。
