...
| 我想… | 直接跳 |
|---|---|
| **接手这个项目**，先读什么 | §12 收尾归档 → **§13 接手会话**（含 7 条新坑）→ **§13.11 当前进度与唯一待办** → **§14 驱动迁到 NAS**；★ **最新一节是 §23**（2026-09-14），**首读先看它** |
...
| 20 | **观测层「沉默 / 响铃」语义收敛**（2026-09-12 深夜 → 09-13；§20.1 `packs` 差集只报变化 + 基线 · §20.3「沉默 = 基线一致」· §20.7 三条首读兑现 · §20.9 `mbf` 排进 `--packs`） | 🔵 决策 + 🔴 症状→解法 |
| 21 | **IYUU 辅种的保存目录从哪来 + 22 条旧根漏网**（2026-09-13；源码三跳：`savePath` = qB 里同 infohash 那条自己的 · 旧根残留实测 · qB「落进可写层」那枚雷） | 🔴 症状→解法 + 🔵 决策 |
| 22 | **`matched_indexers` 恒空 + HDtime 的 429 到底是哪种限流**（2026-09-14；§22.1 根因 · §22.2 修法 · §22.3 429 是 **Prowlarr 发的** · §22.4 `ERR-SVC-17` 改「有据」） | 🔴 症状→解法 + 🔵 决策 |
| 23 | **卷归属复核 + SMB 探针的分辨力 + 一份「日报加各包进度」方案的审订**（2026-09-14；§23.1 双来源复核 · ★★ §23.2 `stat -c %d` **只在 NAS 侧成立** · §23.3 **枚举 ≠ 全量** · §23.5 八条订正，**全未落笔**） | 🔴 症状→解法 + 🔵 决策 |
