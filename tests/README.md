...
2026-09-13 实测：**18 个脚本、688 条断言、0 失败**（本轮新增 `test_linkguard.py` 64 条 + `test_hardlink_link_type.py` 17 条）。
...
| `test_linkguard.py` | 64 | 链接守护 —— 既有那批**硬链接**有没有被写穿（判据 `state.linkguard_*` + 接线 `drive-loop.linkguard_watch`）。钉的不是"算得对"，而是四个**看着像噪声、其实是要命处**的分界：① ★★ **扫了个空绝不许落盘** —— 扫盘失败/根目录写错 ⇒ `cur` 为空，照常落盘的话下一轮拿空基线比对、**每个文件都被报成「新增」**，一条假警报赔掉整条观测的可信度；② ★ **首读不许响**（#47 的规矩：现存差异是"已接受的现状"）；③ ★★ **路径前缀必须带斜杠** —— `/x/甲` 吞掉 `/x/甲乙/…` 会让"哪条种子被写穿"**指错人**，而指错人比不指更坏（会照着错的对象去重建），加「互相是前缀时取最长匹配」；④ ★★ **正文只报数量**（日报是发出去的：不出现路径、不出现 `volume1`、不出现 hash）。另钉：`.!qB`/`.parts`/`.tmp` 不计入基线（重下本来就动它们）、size 变了也抓得到（不只看 mtime）、坏掉的基线**静默重新起锚**而不是全员误报、<br>★★ **`uploading` 必须显式算「静止」** —— 它**不以 `UP` 结尾**，只按 `endswith("UP")` 判会被当成"正在下载"⇒ **误暂停健康做种**。这个坑是**本文件当场抓到**的：生产上跑着的那把看门狗用的是同一个写错判据，只是那 628 条恰好全是 `stalledUP`（看门狗日志 `巡检 40 次：628 条，无下载态`）才没误伤 —— 测试的价值就在这种"它今天没炸、但它就是错的"上。★ 另钉 `checkingUP` **故意算「在动」**（那是校验失败→就地重下的前奏，正是要盯的） |
