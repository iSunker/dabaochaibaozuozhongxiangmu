...
2026-09-12 实测：**12 个脚本、382 条断言、0 失败**。
...
| `test_reconcile.py` | 50 | 观测对账三条判据：**合取 vs 单字面量**（`] Found ` 会吃到 `Found 0 torrents for {`，实测 2231 vs 靶心 1011）、**两口径不是同一个数**（三包共用一个 `farm_root` → 生产口径 other_pack 865/146/1011 而非 0）、**全场无人认领**（1888 条里恰好 1 条，且必须点名路径）。外加接线层四格：`reconcile_watch` **绝不抛**、读不到给 `n/a` 不给 0、差不为 0 时**真的发 alert**、正文里带口径名与留证行 |
